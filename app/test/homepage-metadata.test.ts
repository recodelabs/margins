...
function readIndexDocument() {
  return new DOMParser().parseFromString(readFileSync(indexHtmlPath, "utf8"), "text/html");
}

function metaContent(document: Document, selector: `meta[${string}]`): string | null {
...
describe("homepage metadata", () => {
  it("uses the homepage screenshot for social previews", () => {
    ...
    expect(metaContent(document, 'meta[name="description"]')).toBe(pageDescription);
    expect(document.querySelector('link[rel="canonical"]')?.getAttribute("href")).toBe(
      "https://roughdraft.md/",
    );

    expect(metaContent(document, 'meta[property="og:title"]')).toBe(pageTitle);
    expect(metaContent(document, 'meta[property="og:description"]')).toBe(pageDescription);
    expect(metaContent(document, 'meta[property="og:type"]')).toBe("website");
    expect(metaContent(document, 'meta[property="og:url"]')).toBe("https://roughdraft.md/");
    expect(metaContent(document, 'meta[property="og:image"]')).toBe(previewImageUrl);
    expect(metaContent(document, 'meta[property="og:image:alt"]')).toBe(
      "Roughdraft markdown review workspace",
    );
    expect(metaContent(document, 'meta[property="og:image:width"]')).toBe("3456");
    expect(metaContent(document, 'meta[property="og:image:height"]')).toBe("2234");
    ...
    expect(metaContent(document, 'meta[name="twitter:description"]')).toBe(pageDescription);
    expect(metaContent(document, 'meta[name="twitter:image"]')).toBe(previewImageUrl);
