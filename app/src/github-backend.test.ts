...
describe("GitHubBackend", () => {
  ...
  describe("ETag / conditional-request caching", () => {
    it("first read sends no If-None-Match; a later 304 returns cached content without re-parsing", async () => {
      const fetchMock = vi
        ...
        .mockResolvedValueOnce(
          new Response(
            JSON.stringify({ sha: "v1", content: b64("# A\n"), encoding: "base64" }),
      ...
      expect(headersOf(fetchMock.mock.calls[0][1])["If-None-Match"]).toBeUndefined();
      expect(headersOf(fetchMock.mock.calls[1][1])["If-None-Match"]).toBe('"etag-1"');
    ...
    it("caches the tree listing and serves a 304 from cache", async () => {
      ...
      const fetchMock = vi
        .fn()
        .mockResolvedValueOnce(new Response(tree, { status: 200, headers: { ETag: '"tree-1"' } }))
        .mockResolvedValueOnce(new Response(null, { status: 304, headers: { ETag: '"tree-1"' } }));
      global.fetch = fetchMock as unknown as typeof fetch;

      const bk = backend();
      expect(await bk.listMarkdownPaths()).toEqual(["a.md"]);
      expect(await bk.listMarkdownPaths()).toEqual(["a.md"]);
      expect(headersOf(fetchMock.mock.calls[1][1])["If-None-Match"]).toBe('"tree-1"');
    });
    ...
    it("invalidates the cached read after a successful save", async () => {
      const fetchMock = vi
        ...
        .mockResolvedValueOnce(new Response(JSON.stringify({ content: { sha: "v2" } }), { status: 200 }))
        .mockResolvedValueOnce(
          new Response(
            JSON.stringify({ sha: "v2", content: b64("# B\n"), encoding: "base64" }),
      ...
      expect(headersOf(fetchMock.mock.calls[2][1])["If-None-Match"]).toBeUndefined();
  ...
  describe("rate-limit handling", () => {
    it("throws GitHubRateLimitError with a clear message on a 403 with remaining=0 (not the bare tree error)", async () => {
      ...
      const error = await backend().listMarkdownPaths().catch((e) => e);
    ...
    it("treats a 429 with Retry-After as a rate limit on reads", async () => {
      ...
      const error = await backend().getMarkdownFile("a.md").catch((e) => e);
      expect(error).toBeInstanceOf(GitHubRateLimitError);
    ...
    it("leaves a plain 403 (no rate-limit headers) as the normal labeled error", async () => {
      ...
      global.fetch = fetchMock as unknown as typeof fetch;
      await expect(backend().listMarkdownPaths()).rejects.toThrow(/tree failed \(403\)/i);
    });
    ...
    it("backs off once on a short Retry-After and then succeeds", async () => {
      const fetchMock = vi
        ...
        .mockResolvedValueOnce(
          new Response(JSON.stringify({ tree: [{ path: "a.md", type: "blob" }] }), { status: 200 }),
        );
      global.fetch = fetchMock as unknown as typeof fetch;
    ...
    it("rate-limits saves too (no silent 403 swallow)", async () => {
      ...
      global.fetch = fetchMock as unknown as typeof fetch;
      await expect(backend().saveMarkdownFile("a.md", "# x\n")).rejects.toBeInstanceOf(
        GitHubRateLimitError,
      );
    });
  });
